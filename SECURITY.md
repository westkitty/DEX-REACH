...
- evidence bundles are portable and content-free: they carry identifiers, hashes, signed receipts and sanitized spans, and carry no file content, stdout/stderr, credentials, private keys, secret values, raw owner policy or raw request arguments unless the owner attaches an explicit disclosure. Offline verification answers each claim separately and never emits one overall VERIFIED; node identity, bundle completeness, trace completeness and external side effect are always reported as unproven, because a bundle cannot establish them about itself;
