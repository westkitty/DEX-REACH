...
import { HEARTBEAT_INTERVAL_MS, type AdmissionResult, type WorkRequest } from './work-coordinator.js';
import { coordinatedAcquire, coordinatedHeartbeat, coordinatedRelease } from '../coordinator/client.js';
...
export async function runWithWorkLease(request: WorkRequest, executable: string, args: readonly string[]): Promise<WorkRunResult> {
  const admission = await coordinatedAcquire(request);
  if (admission.status === 'queued') return admission;

  const child = spawn(executable, [...args], { stdio: 'inherit' });
  const timer = setInterval(() => { void coordinatedHeartbeat(admission.lease.id); }, HEARTBEAT_INTERVAL_MS);
  try {
    ...
    clearInterval(timer);
    await coordinatedRelease(admission.lease.id).catch(() => undefined);
  }
}
