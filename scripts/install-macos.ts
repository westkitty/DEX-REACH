...
function plist(label: string, entry: string): string {
...
const services = [
  { label: 'com.stinkyweasel.dex-reach.gateway', entry: 'dist/src/gateway/main.js' },
...
for (const service of services) {
  ...
  await fs.writeFile(target, plist(service.label, service.entry), { mode: 0o600 });
