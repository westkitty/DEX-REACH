...
Fourteen of the fifteen phases in the expansion brief are implemented, swept twice for defects, and proved against a real gateway and node agent running as separate processes. Phase 3 is deliberately absent. **The branch is installed on the primary Mac and has been accepted by a real client.** The coordinator, worker, gateway and node run there as persistent launchd services from runtime code `31bf8a1`. On 2026-09-20 a settings-isolated Claude Code 2.1.267 session, connected only to the configured DEX MCP server and allowed only `reach_fingerprint`, called it once against node `macbook-air.local`: the useful fingerprint payload came back and rendered, the caller-visible trace id `7e58bf5e2dff5b8e96bdcd4ec84f0ee0` was present in the result's `_meta`, and `npm run dex -- trace` resolved that same call through all six stages of `mcp → gateway → node → authorize → execute → receipt`. That was the release blocker, `PND-010`, and it is closed. What remains is publication: CI on the exact head, then the owner's decision on pull request #2. Everything below is checkable against the branch `claude/work-coordinator-cpcug3` in `westkitty/DEX-REACH`, draft pull request #2. Check out the branch tip rather than a fixed hash: documentation commits sit on top of the code they describe.
...
| `31bf8a1` | Reconcile this record with the worker review and the result repair. Documentation only |
| `da11c4f`, `7218f26` | Owner's own follow-up: encode a client's structured-output preference as the contract test's selection rule, and correct a stale comment in the proof harness |
| `a682f62` | Owner's own note: runtime code `31bf8a1` installed and accepted by a real client, closing `PND-010` |
...
**Phase 3, dual-era MCP, is absent, and the gate it was waiting on has now been met.** The brief's own rollback rule says to keep legacy serving until modern serving is proven on the owner's machine, and as of the acceptance above it is: a real client completed a routed call against the installed runtime and the whole six-stage chain resolved. Implementing Phase 3 is now a decision rather than a blocked item, and it is the owner's. The code it needs is already written and shipped unused.
...
**The branch is installed on the primary Mac and accepted by a real client; pull request #2 remains a draft pending CI on the exact head and the owner's merge decision.** It was installed before a full `npm run verify:golden` completed: the reconciled head passed 42 focused coordinator, capacity, work-run and MCP-contract tests on the Mac along with typecheck, the invariant manifest, build, audit and the backend probe, and the full-suite run was queued under machine pressure rather than finished. That is the owner's call on the owner's machine, and it is recorded here so nobody later reads the install as evidence the whole gate passed. The build installed there is runtime code `31bf8a1`; the two commits after it change only a test and a comment, so the installed services are the accepted ones.
...
The Mac is a separate source of evidence, and it should be read as separate. Everything below that happened on the Mac is the owner's own observation, recorded here as that and not re-verified from this container: the install and the services coming back, the accepted `reach_fingerprint` call and its resolved six-stage trace, a Samsung SM-X910 answering harmless identity reads through DEX over wireless ADB, and a proof run there reporting 20 proven and 2 hardware-unavailable, which differs from this container's 19 and 3 because the Android device is attached to his machine and not to this one.
...
The repair was then installed and accepted. The owner put runtime code `31bf8a1` on the Mac, and a settings-isolated Claude Code 2.1.267 session allowed only `reach_fingerprint` called it once: the payload came back and rendered, the trace id was in `_meta` and nowhere else, and `dex trace` resolved the same call through all six stages. So `CI-green`, `installed`, and `proven against a real client on the installed machine` are three different states, and this branch has now demonstrated the distance between the second and the third by failing at it once and then passing.

**The four proof-stale marks on `DEX-INV-005`, `009`, `017` and `021` still stand, and it is worth saying plainly why, because until this acceptance the record said `PND-010` was what would clear them and that was wrong.** They are stale for four separate reasons, none of which `PND-010` asked about. `DEX-INV-005` wants the 22-tool compatibility surface seen from a deployed client rather than a loopback one. `DEX-INV-017` wants a real client listing all 16 first-class actions through the deployed HTTPS ingress, and the ChatGPT connector still shows 12. `DEX-INV-021` wants a live `reach_trust_report` fetched from the deployed gateway, and the accepted session called only `reach_fingerprint`. `DEX-INV-009` wants non-loopback HTTPS and WSS, which no evidence on this machine touches. Two of the four are one more tool call away from the session that already worked; the other two need a real deployment.

### What only the owner can do

1. **Decide what happens to pull request #2.** The release blocker it was held for is gone: `PND-010` closed on the acceptance above. What is left is CI on the exact head and the owner's own call on merging.
2. **Take two more tool calls from that same isolated client session, if the four stale proof marks are worth clearing.** They are stale for four different reasons, and none of those reasons is `PND-010`. `DEX-INV-005` wants the 22-tool compatibility surface seen from a deployed client, and `DEX-INV-021` wants a live `reach_trust_report` from the deployed gateway; both are one more call from the session that already worked, with the tool pre-authorized. `DEX-INV-017` wants that plus a real client listing all 16 actions, which the ChatGPT connector cannot do while it still shows 12. `DEX-INV-009` wants non-loopback HTTPS and WSS, which is a real deployment rather than a call, and is the one of the four that stays out of reach on this machine.
3. **Finish the gate that the earlier install ran ahead of.** `npm run verify:golden` has not completed at an installed head. The Mac passed a focused 42-test gate plus typecheck, invariants, build, audit and probe, and the full-suite run queued under machine pressure rather than finishing.
4. **Refresh the ChatGPT connector's action list.** That client still exposes 12 of the 16 DEX actions, so plan, commit, receipts and the trust report are not reachable from its user interface. The server contract is not the problem. This is `PND-001`.
5. **Authorize the install proof** by running the proof harness on a macOS host with `DEX_REACH_PROOF_ALLOW_INSTALL=1`. This is a separate thing from having installed the branch by hand: `fresh-node-install` only moves off unverified when the harness itself is allowed to perform an install.
6. **Run a real two-agent lease contention** on the Mac, which is what would take `DEX-INV-022` through `026` past regression and installed smoke.
7. **Configure a real rolling budget** on the installed node and confirm a live request is refused when exhausted without changing the owner policy hash. The installed node still has no budget configured.
...
Three pending items remain open in `OPERATIONAL_STATE.md`: `PND-001`, `005` and `009`. `PND-010` closed on the acceptance above, along with `UNV-008` and `UNV-009`. `PND-005` is the one not listed above, and it is not owner-gated: it is a deliberate hold on replacing more compatibility primitives, so that widening the frozen 16-action contract does not get mixed into the migration proof.

The rest closed, and it is worth being precise about how, because several closed on the owner's own hardware rather than on anything provable from here. `PND-002` closed on a real Samsung SM-X910 answering identity reads through DEX. `PND-008` was closed when a real Claude Code client completed a `reach_fingerprint` call against the installed runtime, and that closure should be read narrowly: the call completed and the trace resolved, but the later call against `951818c` showed that completing is not the same as answering, because the payload was missing. The client-reachability question `PND-008` asked was closed then; the result-contract question it did not ask is what `PND-010` carried, and that closed on the later accepted call against the repaired runtime. `PND-016` closed when `DEX-INV-042` was minted for the coordinator daemon transport boundary. `PND-003`, `PND-004` and `PND-007` were not proven; they were reclassified out of pending work and remain recorded as unverified in the `UNV-` entries, which is where a reader should look for the second-machine and Linux gaps. `PND-006`, `011`, `012`, `013` and `015` closed earlier in the expansion.
